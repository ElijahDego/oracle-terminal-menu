...
This is a terminal-based Oracle SQL application that runs prebuilt SQL scripts through a simple shell menu. Instead of manually connecting to Oracle and executing each file one by one, this project uses a Bash script to handle the flow and run everything from one place.
...
To solve this, I used Oracle Instant Client and SQL*Plus, which let me handle database connections directly from the terminal. This setup requires a lot of Oracle client libraries (the `.dylib` and `.jar` files you see in the project folder), but it means the script can take credentials at runtime, use them for that session, and then discard them when the program ends.
...
```
.
├── menu.sh              # Main shell script and entry point
├── create_tables.sql    # Database schema setup
├── insert_data.sql      # Sample data insertion
├── views.sql            # Oracle view definitions
├── queries.sql          # Advanced SQL queries
└── README.md            # Project documentation
```

**Note:** The Oracle Instant Client libraries (`.dylib`, `.jar` files) are required locally to run the project but are typically not included in the GitHub repository since they are environment dependencies. I have included it in the repository for access

## Prerequisites

- Access to an Oracle database
- SQL*Plus / Oracle Instant Client installed
...
This project was created as I was completing a database course assignment. I wanted to figure out how Use oracle database from the terminal as well as I wanted to know how to make it so that the user could log in with their individual Oracle DB logins. Made just for fun! Feel free to reach out if you have any notes that could work better than this! 😊
